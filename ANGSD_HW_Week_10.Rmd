...
normalized.sce <- SingleCellExperiment(list(counts=normalized_counts_matrix))
clusters <- quickCluster(normalized.sce, min.size=100)
normalized.sce <- computeSumFactors(normalized.sce, cluster=clusters)

#library(scater)
#rownames(sce) <- uniquifyFeatureNames(rowData(normalized.sce)$ID, rowData(normalized.sce)$Symbol)
#head(rownames(sce))
